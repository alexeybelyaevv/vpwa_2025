<template>
  <div class="sl-chat">
    ...
    <q-scroll-area ref="scrollArea" class="q-pa-md" style="flex: 1" :horizontal="false">
      <sl-messages-list
    ...
    <div class="row items-center q-pa-sm input-container">
      ...
      <q-btn class="q-ml-sm send-btn" label="Send" @click="sendMessage" />
    </div>
  </div>
</template>
...
const chatTitle = computed(() =>
  chatCommandsStore.state.currentChannel
    ? chatCommandsStore.getChannelByTitle(chatCommandsStore.state.currentChannel)?.title ||
      'No Channel Selected'
    : 'No Channel Selected',
);
