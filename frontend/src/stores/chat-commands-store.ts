...
export const useChatStore = defineStore('chat', () => {
  ...
  function kick(nickName: string) {
    ...
    if (channel.type === 'private' && isAdmin) {
      ...
      let voteSet = channel.kicks[nickName];
      if (!voteSet) {
        voteSet = new Set<string>();
        channel.kicks[nickName] = voteSet;
      }
      if (voteSet.has(state.currentUser)) {
        if (!state.messages[channelTitle]) state.messages[channelTitle] = [];
        state.messages[channelTitle].push({
          id: Date.now(),
          chatId: channelTitle,
      ...
      voteSet.add(state.currentUser);
      if (!state.messages[channelTitle]) state.messages[channelTitle] = [];
      state.messages[channelTitle].push({
        id: Date.now(),
        chatId: channelTitle,
        senderId: state.currentUser,
        text: `${state.currentUser} voted to kick ${nickName} (${voteSet.size}/3)`,
      });
      if (isAdmin || voteSet.size >= 3) {
        channel.members = channel.members.filter((m) => m !== nickName);
        channel.banned.push(nickName);
        if (!state.messages[channelTitle]) state.messages[channelTitle] = [];
        state.messages[channelTitle].push({
  ...
  function generateSlug(value: string): string {
    ...
    const existing = state.channels.map((chat) => chat.slug);
